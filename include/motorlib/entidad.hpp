...
private:
  ...
  // notificar coordenadas del sonambulo
  bool mensaje2 = false;
  ...
  Entidad* EntidadSonambulo;


  vector<vector<unsigned char>> visionAux;

  Action ActionSent = actIDLE;
...
//  Action think(int acc, vector<vector<unsigned char>> vision, int level);
  ...
  Action putLastAction() { return last_action; }
  ...
  int fixBateria_sig_accion(unsigned char celdaJugador, unsigned char celdaSonambulo, Action accion);
  ...
  void SetSonambulo (Entidad *x) {EntidadSonambulo = x;}
};
#endif
